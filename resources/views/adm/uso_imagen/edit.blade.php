...
@section('cuerpo')
  ...
  <div class="row">
    <div class="col s8">
  		<h4 style="color: var(--main-color-links)">{!!$data->title_es!!}</h4>
  		{{-- @include('adm.layouts.report') --}}
  	</div>
  	<div class="col s4">
      <a href="{{ route(strtolower($model).'.show', ['id' => $data->uso_id]) }}" class="btn-floating btn-medium waves-effect waves-light blue right ml-3x"><i class="material-icons">keyboard_backspace</i></a>
  		<a href="{{ route(strtolower($model).'.create', ['id' => $data->uso_id]) }}" class="btn-floating btn-medium waves-effect waves-light green right ml-3x"><i class="material-icons">add</i></a>
  	</div>
  </div>
  ...
  {{ csrf_field() }}
      ...
      {{--
        ...
        <div class="input-field col m4 s12">
          {!!Form::label('orden', trans('translator.main-order'))!!}
          {!!Form::text('orden',$data->orden,['class'=>'validate'])!!}
        </div>
  	</div>
