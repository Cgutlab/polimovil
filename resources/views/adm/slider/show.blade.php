	...
	<div class="col s12">
		<table class="highlight bordered">
			<thead>
				...
				<td>@lang('translator.main-text')</td>
				...
				<tr>
					<td>
						@if(file_exists(public_path().'/img/'.strtolower($model).'/'.$dat->image) && $dat->image)
							<img class="materialboxed" src="{{asset('img/'.strtolower($model).'/'.$dat->image)}}" alt="{{$dat->title_es}}" class="responsive-img" style="max-height: 50px; max-width: 160px;">
						@else
