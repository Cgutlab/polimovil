	...
	<div class="col s12">
		<table class="highlight bordered">
			...
			<tbody>
				...
				<tr>
					<td>
						@foreach($dat->usoimage as $img)
						@if(file_exists(public_path().'/img/uso/'.$img->image) && $img->image)
							<img class="materialboxed" src="{{asset('img/uso/'.$img->image)}}" alt="{{$img->title_es}}" class="responsive-img" style="max-height: 80px; max-width: 240px;">
						@else
            	...
            	<div class="flex-center" style="display: flex; justify-content: right; align-items: center; margin-bottom: 5px;">
            				<a href="{{ route('uso_imagen.show', ['id' => $dat->id]) }}" class="btn-floating btn-small waves-effect waves-light blue ml-3x"><i class="far fa-images"></i></a>
