...
@section('cuerpo')
  ...
  <div class="row">
    <div class="col s8">
  	...
  	<div class="col s4">
      <a href="{{ route('maq_'.strtolower($model).'.show', ['id' => $master->id]) }}" class="btn-floating btn-medium waves-effect waves-light blue right ml-3x"><i class="material-icons">keyboard_backspace</i></a>
  ...
  {!!Form::open(['route'=>['maq_'.strtolower($model).'.store'], 'method'=>'POST', 'files' => true])!!}
  {{ csrf_field() }}
  	<div class="row">
        <div class="file-field input-field col s12 right hide">
          {!! Form::label('maquina_id', trans('translator.main-foreign')) !!}
          {!! Form::select('maquina_id', [
            $master->id => $master->title_es
          ], ['class'=>'validate', 'required', 'id' => 'maquina_id']) !!}
        </div>
