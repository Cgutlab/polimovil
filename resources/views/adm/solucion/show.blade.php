...
<div class="row">
	<div class="col s12">
		<table class="highlight bordered">
			<thead>
				<td>@lang('translator.main-icon')</td>
			...
			<tbody>
				...
				<tr>
					<td>
						@if(file_exists(public_path().'/img/'.strtolower($model).'/'.$dat->icon) && $dat->icon)
							<img class="materialboxed" src="{{asset('img/'.strtolower($model).'/'.$dat->icon)}}" alt="{{$dat->title_es}}" class="responsive-img" style="max-height: 80px; max-width: 240px;">
						@else
							<img src="{{asset('img/logo/'.$default->image)}}" alt="{{$default->image}}" class="responsive-img" style="width: 50px;">
						@endif
					</td>
					<td>
						@forelse ($dat->imagenes as $item)
							@if(file_exists(public_path().'/img/'.strtolower($model).'/'.$item->image) && $item->image)
								<img class="materialboxed" src="{{asset('img/'.strtolower($model).'/'.$item->image)}}" alt="{{$item->title_es}}" class="responsive-img" style="max-height: 50px; max-width: 160px;">
							@else
								<img src="{{asset('img/logo/'.$default->image)}}" alt="{{$default->image}}" class="responsive-img" style="width: 50px;">
							@endif
							@break
            	...
            	<div class="flex-center" style="display: flex; justify-content: center; align-items: center;">
								<a href="{{ route(strtolower($model).'img.show', ['id' => $dat->id]) }}" class="btn-floating btn-small waves-effect waves-light blue ml-3x"><i class="far fa-images"></i></a>
