  ...
  {{ csrf_field() }}
  	<div class="row">
        ...
        <div class="file-field input-field col m4 s12">
          ...
          <div class="file-path-wrapper">
              {!!Form::text('', isset($repeat['icon']) ? $repeat['icon'] : old('icon'), ['placeholder' => trans('translator.main-image-icon'), 'class'=>'file-path validate']) !!}
              @if(isset($repeat['icon'])) {!!Form::text('icon', isset($repeat['icon']) ? $repeat['icon'] : old('icon'), ['class' => 'hide']) !!} @endif
          </div>
        </div>
