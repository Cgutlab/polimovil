...
<div class="row">
	...
	<div class="col s4">
		<a href="{{ route('item_category.index') }}" class="btn-floating btn-medium waves-effect waves-light blue right ml-3x"><i class="material-icons">keyboard_backspace</i></a>
		...
		<table class="highlight bordered">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						@if(file_exists(public_path().'/img/'.strtolower($model).'/'.$dat->image) && $dat->image)
							<img class="materialboxed" src="{{asset('img/'.strtolower($model).'/'.$dat->image)}}" alt="{{$dat->title_es}}" class="responsive-img" style="width: 50px;">
						@else
            	...
            	<div class="flex-center" style="display: flex; justify-content: center; align-items: center;">
							<a href="{{ route('item_'.strtolower($model) . '.edit', ['id' => $dat->id, 'master' => $master]) }}" class="btn-floating btn-small waves-effect waves-light orange right ml-3x"><i class="fas fa-pencil-alt"></i></a>
